...
import { X, Search, Copy, Check, ChevronDown, ChevronRight, Download } from 'lucide-react';
...
export const MetadataModal: React.FC<MetadataModalProps> = ({ isOpen, onClose, filename, data }) => {
      ...
      >
        ...
        <div className="flex-none px-6 py-4 border-b border-white/[0.06] bg-white/[0.02] flex items-center justify-between">
          <div className="flex flex-col overflow-hidden">
            <h3 className="text-sm font-semibold text-foreground tracking-tight">Image Inspector</h3>
            <span className="text-xs text-foreground-muted font-mono mt-0.5 truncate">{filename}</span>
          </div>
          <Button variant="icon" onClick={onClose} className="hover:bg-white/[0.08]">
            <X size={18} />
          </Button>
        </div>
        ...
        <div className="flex-none px-6 py-3 border-b border-white/[0.06] bg-[#0a0a0c] flex gap-3">
          ...
          <Button variant="secondary" onClick={handleExport} className="gap-2" tooltip="Export as JSON">
            <Download size={14} />
              ...
              <div key={group.id} className="group-section">
                ...
                >
                  {collapsedGroups.has(group.id) ? (
                    <ChevronRight size={14} className="text-foreground-muted group-hover:text-foreground" />
                  ) : (
                    <ChevronDown size={14} className="text-foreground-muted group-hover:text-foreground" />
                  )}
                    ...
                    {group.entries.map((entry, idx) => (
                      ...
                      >
                        ...
                        <div className="w-full sm:w-48 p-3 sm:py-2 sm:px-4 bg-white/[0.01] border-b sm:border-b-0 sm:border-r border-white/[0.04] flex items-center">
                          <span className="text-xs text-foreground-muted font-medium break-words">{entry.key}</span>
                        </div>
                        
                        {/* Value Column */}
                        <div className="flex-1 p-3 sm:py-2 sm:px-4 flex items-center relative min-w-0 pr-12">
                          <div className={`text-xs font-mono text-foreground/90 ${entry.isLong ? 'whitespace-pre-wrap break-words leading-relaxed py-1' : 'truncate'}`}>
                            {entry.value}
                          </div>
                          ...
                          <div className="absolute right-3 top-1/2 -translate-y-1/2 opacity-0 group-hover/row:opacity-100 transition-opacity bg-[#0a0a0c] shadow-xl rounded-md">
                            <Button 
                              variant="icon" 
                              className="h-7 w-7" 
                              onClick={() => handleCopy(String(entry.value), `${group.id}-${entry.key}`)}
                              tooltip="Copy Value"
                            >
        ...
        <div className="flex-none px-6 py-4 bg-white/[0.02] border-t border-white/[0.06] flex justify-between items-center">
           <span className="text-[10px] text-foreground-subtle">
             {filteredData.reduce((acc, g) => acc + g.entries.length, 0)} tags visible
           </span>
