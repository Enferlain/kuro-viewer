...
const generateMockImages = (): ImageFile[] => {
  const images = [];
  const subjects = ['mountain', 'river', 'city', 'abstract', 'technology', 'space'];
  for (let i = 0; i < 15; i++) {
    const id = i + 1;
...
const App: React.FC = () => {
    ...
    return [
      {
        ...
        entries: [
          ...
          { key: 'File Size', value: `${(2.4 + (seed % 10) * 0.3).toFixed(2)} MB` },
          { key: 'Dimensions', value: `${width}x${height}` },
          { key: 'Date Created', value: new Date().toISOString().split('T')[0] },
      ...
      },
      {
        ...
        entries: [
          { 
            key: 'Prompt', 
            value: 'masterpiece, best quality, ultra-detailed, 8k, cyberpunk city, neon lights, rain, reflection, volumetric lighting, cinematic composition, intricate details, highly detailed texture, ray tracing', 
            isLong: true 
          },
          { 
            key: 'Negative Prompt', 
            value: 'low quality, worst quality, bad anatomy, bad hands, text, error, missing fingers, extra digit, fewer digits, cropped, worst quality, low quality, normal quality, jpeg artifacts, signature, watermark, username, blurry',
            isLong: true
          },
        ]
      },
  ...
  const handleZoomOut = () => setViewerState(prev => ({ 
    ...prev, 
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case 'N':
          e.preventDefault();
          setActiveFilter(prev => prev === FilterType.NOISE ? FilterType.NONE : FilterType.NOISE);
          break;
        case 'p':
        case 'P':
          e.preventDefault();
          setActiveFilter(prev => prev === FilterType.PCA ? FilterType.NONE : FilterType.PCA);
          break;
        case '0':
          e.preventDefault();
          handleResetView();
  ...
  return (
    <div className="flex flex-col h-screen w-screen bg-background-deep text-foreground font-sans overflow-hidden">
      ...
      {/* 1. Header / Toolbar */}
      <div className={`transition-all duration-300 ease-in-out overflow-hidden ${isToolbarVisible ? 'h-14 opacity-100' : 'h-0 opacity-0'}`}>
        <Toolbar 
