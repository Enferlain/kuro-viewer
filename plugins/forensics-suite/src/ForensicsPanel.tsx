...
import { DEFAULT_FORENSICS_HOTKEYS, sanitizeHotkey } from "./plugin";

interface ForensicsPanelProps {
	state: ForensicsPluginState;
	onModeChange: (next: ForensicsModeId) => void;
							...
							<label className="inline-flex items-center gap-2 cursor-pointer">
								<input
									...
									onChange={(event) =>
										onPcaChange({
											...state.pca,
											linearize: event.target.checked,
										})
									}
									className="accent-accent cursor-pointer"
								/>
						...
						<div className="mt-2 grid grid-cols-2 gap-3">
							<div>
								...
								<Dropdown
									...
									onChange={(mode) =>
										onTextureChange({ ...state.texture, mode })
									}
									options={[
