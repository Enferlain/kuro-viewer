...
pub fn validate_plugin_manifest(manifest: &PluginManifest) -> Result<(), String> {
	let schema_major = parse_semver_major(&manifest.schema_version)
	...
	let plugin_api_major = parse_semver_major(&manifest.api_version)
		.map_err(|e| format!("api_version {e}"))?;
	...
	parse_semver_major(&manifest.min_host_version)
...
fn parse_semver_major(version: &str) -> Result<u64, String> {
	...
	if !parts.iter().all(|p| !p.is_empty() && p.chars().all(|ch| ch.is_ascii_digit())) {
		return Err(format!("must contain numeric semver parts, got '{version}'"));
	}
	parts[0]
		.parse::<u64>()
...
fn validate_relative_path(value: &str, label: &str) -> Result<(), String> {
	...
	for segment in value.split(['/', '\\']) {
		if segment.is_empty() || segment == "." || segment == ".." {
			return Err(format!("{label} must not contain '.', '..', or empty segments"));
		}
	}
